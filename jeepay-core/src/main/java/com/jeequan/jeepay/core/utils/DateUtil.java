...
public class DateUtil {
  ...
  private static final long nd = 1000 * 24 * 60 * 60; //每天毫秒数
  private static final long nh = 1000 * 60 * 60; //每小时毫秒数
  private static final long nm = 1000 * 60; //每分钟毫秒数
  ...
  /**
   * 验证字符串是否为日期 验证格式:YYYYMMDD、YYYY_MM_DD、YYYYMMDDHHMISS、YYYYMMDD_HH_MI、YYYY_MM_DD_HH_MI、YYYYMMDDHHMISSSSS、YYYY_MM_DD_HH_MI_SS
   *
   * @param strTime
  ...
  public static boolean validateIsDate(String strTime) {
    ...
    if (date != null) {
      System.out.println("生成的日期:" + DateUtil
              .parseDateToStr(date, DateUtil.DATE_TIME_FORMAT_YYYY_MM_DD_HH_MI_SS, "--null--"));
      return true;
    }
    return false;
  }
  ...
  public static long[] getDistanceTime(Date one, Date two) {
    ...
    try {
      ...
      year = DateCalcUtil.yearsBetween(DateCalcUtil.sdf.format(start),DateCalcUtil.sdf.format(end));
     // month =DateUtil1.getIntervalMonths(start,end);
      week = diff / nd/7;
      day = diff / nd;
      hour = diff / nh;
      min = diff / nm;
    ...
    long[] times = {year, month, week, day, hour, min, sec};
    return times;
  }
