...
@Component
public class WsPayOrderMatchServer {

  private final static Logger logger = LoggerFactory.getLogger(WsPayOrderMatchServer.class);
   ...
   */
  private static Map<String, Set<WsPayOrderMatchServer>> wsOrderIdMap = new ConcurrentHashMap<>();
  ...
  public void onOpen(Session session, @PathParam("payOrderId") String payOrderId,
    ...
    try {
      ...
      Set<WsPayOrderMatchServer> wsServerSet = wsOrderIdMap.get(payOrderId);
      if (wsServerSet == null) {
  ...
  public static void sendMsgByOrderId(String payOrderId, String msg) {
    ...
    try {
      ...
      Set<WsPayOrderMatchServer> wsSet = wsOrderIdMap.get(payOrderId);
      if (wsSet == null || wsSet.isEmpty()) {
        logger.info("payOrderId[{}] 无ws监听客户端", payOrderId);
        return;
      }

      for (WsPayOrderMatchServer item : wsSet) {
        try {
