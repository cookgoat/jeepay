 ...
 */
package com.jeequan.jeepay.reseller.ctrl.sysuser;
...
import com.jeequan.jeepay.reseller.service.AuthService;
...
public class SysUserController extends CommonCtrl {
	...
	public ApiRes list() {
		...
		condition.eq(SysUser::getSysType, CS.SYS_TYPE.RESELLER);
	...
	public ApiRes add() {
		...
		sysUserService.addSysUser(sysUser, CS.SYS_TYPE.RESELLER);
		return ApiRes.ok();
	}
...
//	@RequestMapping(value="/modifyPwd", method = RequestMethod.PUT)
	public ApiRes authInfo() {
		...
		sysUserAuthService.resetAuthInfo(opSysUserId, null, null, opUserPwd, CS.SYS_TYPE.RESELLER);
		return ApiRes.ok();
	}
	...
	public ApiRes update(@PathVariable("recordId") Long recordId) {
		...
		if (resetPass != null && resetPass) {
			...
			sysUserAuthService.resetAuthInfo(sysUser.getSysUserId(), null, null, updatePwd, CS.SYS_TYPE.RESELLER);
			// 删除用户redis缓存信息
	...
	public ApiRes delete(@PathVariable("recordId") Long recordId) {
		...
		SysUser mchUserDefault = sysUserService.getOne(SysUser.gw()
				...
				.eq(SysUser::getSysType, CS.SYS_TYPE.RESELLER)
		...
		// 删除用户
		sysUserService.removeUser(sysUser, CS.SYS_TYPE.RESELLER);
