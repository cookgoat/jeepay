 ...
 */
package com.jeequan.jeepay.reseller.service;
...
import com.jeequan.jeepay.reseller.config.SystemYmlConfig;
...
public class AuthService {
    ...
    public String auth(String username, String password){
        ...
        if(sysUser.getIsAdmin() != CS.YES && sysEntitlementMapper.userHasLeftMenu(sysUser.getSysUserId(), CS.SYS_TYPE.RESELLER) <= 0){
            throw new BizException("当前用户未分配任何菜单权限，请联系管理员进行分配后再登录！");
        }

        //查询核销商账户信息
        QueryWrapper<Reseller> wrapper = new QueryWrapper();
        wrapper.lambda().eq(Reseller::getInitUserId,sysUser.getSysUserId());
        Reseller reseller = resellerService.getOne(wrapper);
        //判断账户状态是否可用
        if(reseller != null && StringUtils.equalsIgnoreCase(AccountStatusEnum.DISABLE.getCode(),reseller.getStatus())){
            throw new BizException("当前核销商未启用！");
        }
        // 放置权限集合
